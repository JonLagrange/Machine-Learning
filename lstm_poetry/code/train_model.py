...
# 按诗的字数排序  
poetrys = sorted(poetrys,key=lambda line: len(line),reverse = True)
...
class DataSet(object):
    ...
    def data_batch(self,start,end):
        ...
        xdata = np.full((end - start,length), word_num_map[' '], np.int32)  # end-start行length列全部填充为6109
...
def train_neural_network():
    ...
    tvars = tf.trainable_variables()  # 获取所有可训练的向量,tf.trainable_variables返回所有当前计算图中 在获取变量时未标记trainable=False的变量集合
